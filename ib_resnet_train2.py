...
def train(train_loader, model, criterion, optimizer, epoch, writer, kl_alpha, no_ib):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        if args.no_ib:
            ...
            if not args.wib:
                ...
                output, wib_total = model(input_var)
                writer.add_scalar('train_wid', wib_total.data, ite)
            ...
            if not args.wib:
                ...
                output, kl_total, wib_total = model(input_var)
                writer.add_scalar('train_kld', kl_total.data, ite)
                writer.add_scalar('train_wid', wib_total.data, ite)
                # print('wib_total: ', wib_total)
        ...
        if args.wib_fac > 0 and args.wib:
            loss += wib_total * args.wib_fac
        ...
        if args.wib:
            wib_losses.update(wib_total.data.item() * args.wib_fac, input.size(0))
            # kld_meter.update(kl_total.data.item(), input.size(0))
    ...
    print('Loss {loss.val:.4f} ({loss.avg:.4f})\t'
          ...
          'wibloss {wibloss.val:.4f} ({wibloss.avg:.4f})\t'.format(
        loss=losses, klloss=kl_losses, wibloss=wib_losses ))
